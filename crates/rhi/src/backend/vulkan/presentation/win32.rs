        ...
        match self.try_acquire(device) {
            ...
            Ok(None) => {
                ...
                *self
                ...
                if let Err(info) = self.shared.register_loss_waker(waker) {
                ...
                if !self.acquire_wake.active.swap(true, Ordering::AcqRel) {
                    ...
                    if std::thread::Builder::new()
                        ...
                        .spawn(move || wake_acquire_until_repolled(weak))
                        .is_err()
                    {
...
fn wake_acquire_until_repolled(wake: Weak<AcquireWake>) {
    loop {
        ...
        let waker = wake.waker.lock().unwrap_or_else(|p| p.into_inner()).clone();
