...
impl GlFormat {
    ...
    const fn is_non_compressed_core_baseline_for(self, _profile: GlFamilyProfile) -> bool {
        matches!(
            self,
    ...
    const fn is_supplied_by_extension(self, extension: GlKnownExtension) -> bool {
        match extension {
            GlKnownExtension::CompressedTextureS3tc => matches!(
                self,
            ...
            GlKnownExtension::CompressedTextureBptc => matches!(
                self,
                Self::Bc6hRgbUfloat | Self::Bc6hRgbSfloat | Self::Bc7RgbaUnorm | Self::Bc7RgbaSrgb
            ),
            GlKnownExtension::CompressedTextureRgtc => matches!(
                self,
            ...
            GlKnownExtension::CompressedTextureEtc => matches!(
                self,
                Self::Etc2Rgb8Unorm
                    | Self::Etc2Rgb8Srgb
                    | Self::Etc2Rgba8Unorm
    ...
    pub(crate) fn record(
        ...
        if capabilities.format.compressed_info().is_some()
            && (capabilities.resource_kind != GlFormatResourceKind::Texture
                ...
                || capabilities.copy_source
                || capabilities.copy_destination)
        {
        ...
        for facts in self.entries.values() {
            ...
            let maximum = match facts.resource_kind {
                ...
                GlFormatResourceKind::Texture => match facts.format {
                    ...
                    | GlFormat::Depth32Float => limits.max_depth_texture_samples,
