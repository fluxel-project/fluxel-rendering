...
fn timer_capability_row_matches_the_recorded_counter_width() {
    ...
    if supports {
        // An enabled row must not reject the interval it claims to support;
        // a failure here means the entry points cannot really be called.
        panic!("timer capability enabled but begin failed: {error:?}");
    }
    assert!(
