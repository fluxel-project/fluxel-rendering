...
pub struct TextureDescriptor {
    ...
    /// The set of formats allowed for alternate-format views, sorted and
    /// deduplicated, and never containing the base format.
...
impl TextureView {
    ...
    pub fn extent(&self) -> Extent3d {
        ...
        let base = descriptor.extent;
        let shift = self.descriptor().base_mip;
        Extent3d {
            width: (base.width >> shift).max(1),
            height: (base.height >> shift).max(1),
            depth: (base.depth >> shift).max(1),
        }
    }
