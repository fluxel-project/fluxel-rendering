...
use crate::rhi::{RhiError, RhiErrorKind};
...
pub(super) fn to_rhi(error: &WinError, operation: &'static str) -> RhiError {
    ...
    RhiError::new(
        failure.kind(),
        format!("Direct3D 12 call failed: {error} (HRESULT {:#010x})", error.code().0),
    )
    .at(operation)
}
...
mod tests {
    ...
    fn a_reset_or_internal_driver_failure_is_terminal() {
        ...
        assert!(
            NativeFailure::classify(&error(DXGI_ERROR_DRIVER_INTERNAL_ERROR.0)).is_terminal()
        );
    }
