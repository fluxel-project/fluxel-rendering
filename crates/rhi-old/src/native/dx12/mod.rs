...
//! synchronization object that could not be created. A `InvalidUsage` returned
//! from this module means the façade missed a portable check, and that is a bug
//! in the façade rather than a fact about the request.
//!
...
//! second, DX12-shaped architecture for the next platform to copy. The
//! public handles (`Buffer`, `Texture`, `RasterPipeline`, …) each wrap an
//! `Arc<dyn …Backend>`, and the Direct3D object is a field of the concrete
//! implementation behind that trait object.
//!
//! # Lifetime
//!
...
//! `fluxel-host`; this module consumes a `PresentationTarget` and never creates a
//! window or a swapchain-before-request. Format/route facts are *not* invented
//! here — they are read from the device and recorded as instance data, because a
//! capability that is asserted from a trait's presence rather than queried is
//! the exact failure API v1 exists to prevent.
//!
//! # Where this tree stands, stated plainly
//!
//! This is a partial implementation and no claim of DX12 support exists until
//! the shared contract suite and a real Windows run close on one revision. Only
//! the native boundary is written so far. The order the rest arrives in is fixed
//! by a dependency, not by preference: DX12 answers every capability question
//! (`CheckFeatureSupport`, format support, sampler feedback, resource-binding
//! tiers) through a live device, so device creation has to land before adapter
//! enumeration can report honest instance data — a provider that enumerated
//! first would have to invent the facts it reported.
