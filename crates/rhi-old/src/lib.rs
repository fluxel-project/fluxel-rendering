...
mod common;
/// Fluxel RHI API v1: the frozen portable interface, rewritten from the
/// specification in `documents/rhi-design/01`-`08`.
///
/// This is the target public surface, and the only module tree a caller should
/// name. It is being built contract-first: the types, the validation, and the
/// refusal paths land before the lowering, which arrives under `crate::backend`.
/// A verb whose body is still `unimplemented!()` has its signature frozen and
/// its implementation outstanding.
pub mod api;
/// Fluxel RHI API v1: the earlier portable interface, kept while its callers
/// migrate to [`api`].
///
/// This tree and the crate-root facade above it are retired once every caller
/// names [`api`] and `crate::backend` lowers it.
pub mod rhi;
