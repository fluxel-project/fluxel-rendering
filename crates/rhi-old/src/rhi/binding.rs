...
impl BindGroupLayout {
    ...
    pub(crate) fn backing(&self) -> Arc<dyn BindGroupLayoutBackend> {
...
impl BindingResource {
    ...
    /// Every device-affine object this value names, paired with its owner.
    ///
    /// The point is that a caller checking a whole packet against one device
    /// does not have to match on six variants itself: the match lives here, so
    /// adding a variant is what forces the check to be updated rather than
    /// letting a new resource class slip past unexamined.
    pub(crate) fn device_objects(&self) -> Vec<(ObjectId, DeviceIdentity)> {
        match self {
            Self::Buffer(binding) => {
                vec![(binding.buffer.id(), binding.buffer.device_identity())]
            }
            Self::Texture(view) => vec![(view.id(), view.device_identity())],
            Self::Sampler(sampler) => vec![(sampler.id(), sampler.device_identity())],
            Self::BufferArray(bindings) => bindings
                .iter()
                .map(|binding| (binding.buffer.id(), binding.buffer.device_identity()))
                .collect(),
            Self::TextureArray(views) => views
                .iter()
                .map(|view| (view.id(), view.device_identity()))
                .collect(),
            Self::SamplerArray(samplers) => samplers
                .iter()
                .map(|sampler| (sampler.id(), sampler.device_identity()))
                .collect(),
        }
    }
...
impl BindGroupDescriptor {
    ...
    /// The entries sorted by ascending slot id.
    ///
    /// The order a caller filled its entries in carries no meaning, so a bind
    /// group's identity cannot depend on it (rhi-design section 48.1). Sorting
    /// here is also what lets a validator catch a repeated slot by comparing
    /// neighbours instead of comparing every pair.
    pub(crate) fn canonicalized(&self) -> Self {
        let mut canonical = self.clone();
        canonical.entries.sort_by_key(|entry| entry.slot);
        canonical
    }
}
...
impl BindGroup {
    ...
    /// The backing this handle retains, for the device's retirement registry.
    pub(crate) fn backing(&self) -> Arc<dyn BindGroupBackend> {
        Arc::clone(&self.inner)
    }
...
/// Validates a bind group's entries against itself and against its layout.
///
/// `entries` must be the canonical form of the descriptor's entry list, so a
/// repeated slot appears as two neighbours. Two entries claiming one slot is a
/// contradiction rather than a duplicate: a packet that fills one slot twice
/// has no defined contents, so this is rejected rather than resolved by keeping
/// the last entry.
///
/// Filling only part of the layout is legal here. A layout slot with no entry
/// is a hole, and whether a hole is allowed is a property of the pipeline that
/// consumes the group, not of the group itself.
pub(crate) fn validate_bind_group_entries(
    entries: &[BindGroupEntry],
    layout_entries: &[BindingSlot],
) -> RhiResult<()> {
    for window in entries.windows(2) {
        if window[0].slot == window[1].slot {
            return Err(RhiError::new(
                RhiErrorKind::InvalidUsage,
                format!("bind group fills slot {} twice", window[0].slot.get()),
            ));
        }
    }
    for entry in entries {
        let Some(slot) = layout_entries.iter().find(|slot| slot.slot == entry.slot) else {
            return Err(RhiError::new(
                RhiErrorKind::InvalidUsage,
                format!(
                    "bind group fills slot {} which its layout does not declare",
                    entry.slot.get()
                ),
            ));
        };
        if !entry.resource.matches_kind(&slot.kind) {
            return Err(RhiError::new(
                RhiErrorKind::InvalidUsage,
                format!(
                    "bind group fills slot {} with a resource of the wrong class",
                    entry.slot.get()
                ),
            ));
        }
        // A scalar slot takes exactly one resource and an array slot takes the
        // count the layout fixed. Letting a shorter array through would make
        // the shader's element indices read past what the packet binds. The
        // form matters as well as the count: a one-element array is not the
        // same declaration to a backend as a scalar.
        let expected = slot.count.elements();
        let is_array = matches!(slot.count, BindingCount::Fixed(_));
        if entry.resource.is_array() != is_array || entry.resource.len() != expected as usize {
            return Err(RhiError::new(
                RhiErrorKind::InvalidUsage,
                format!(
                    "bind group fills slot {} with {} resources but its layout declares {}",
                    entry.slot.get(),
                    entry.resource.len(),
                    expected
                ),
