...
use super::capability::{DeviceLimits, EnabledCapabilities};
use super::platform::{
...
pub(crate) struct RecorderLimits {
    ...
    /// The device's whole capability contract.
    ///
    /// The three fields above are the facts the recorder reads on almost every
    /// command, lifted out of this contract once so the hot path does not walk
    /// a limit map. A copy's row pitch is not that kind of fact: section 34.2
    /// makes it a property of the *route*, and a route is keyed by dimension,
    /// format, and aspect. Caching it as one scalar would answer for every
    /// route at once, so it is asked of the contract per copy instead.
    pub capabilities: EnabledCapabilities,
}

impl RecorderLimits {
    /// Reads the recorder-relevant facts out of a device's capabilities.
    ///
    /// `compute_enabled` is passed rather than derived because a device's
    /// compute feature and its compute limit are declared separately, and a
    /// recorder must reflect what the device reported rather than infer it.
    pub(crate) fn from_capabilities(
        capabilities: &EnabledCapabilities,
        compute_enabled: bool,
    ) -> Self {
        let limits: &DeviceLimits = capabilities.limits();
        Self {
...
impl CommandRecorder {
    ...
    /// Returns the recorder to its open state as a scope closes.
    ///
    /// A poisoned recorder stays poisoned. Section 29.3 makes a backend
    /// recording failure a one-way transition, and a scope's own successful
    /// close is not an exception to it: the close reaches the backend and
    /// succeeds precisely because only the command inside the scope failed.
    /// Assigning `Open` unconditionally here would let that successful `end()`
    /// undo the poison, and the corrupt stream would become finalizable again.
    fn reopen_after_scope(&mut self) {
    ...
    pub fn copy_buffer_to_texture(&mut self, copy: &BufferTextureCopy) -> RhiResult<()> {
        ...
        let uses = validate::buffer_texture_uses(copy, false, &self.limits.capabilities)
    ...
    pub fn copy_texture_to_buffer(&mut self, copy: &BufferTextureCopy) -> RhiResult<()> {
        ...
        let uses = validate::buffer_texture_uses(copy, true, &self.limits.capabilities)
