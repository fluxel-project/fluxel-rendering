...
use super::capability::EnabledCapabilities;
use super::platform::{
...
impl PipelineInterface {
    ...
    pub(crate) fn backing(&self) -> Arc<dyn PipelineInterfaceBackend> {
...
impl InterfaceLimits {
    /// Reads these limits out of a device's capability contract.
    ///
    /// The façade and every backend derive their limits this way, so a device
    /// cannot be measured against one set of numbers by the caller that decides
    /// legality and another by the implementation that lowers it.
    pub(crate) fn from_capabilities(capabilities: &EnabledCapabilities) -> Self {
        Self {
            // A device that reports no MaxBindGroups cannot bind a group at all,
            // so zero is the honest reading rather than a permissive default.
            max_bind_groups: capabilities.limit(LimitKey::MaxBindGroups).unwrap_or(0),
            max_dynamic_uniform_buffers: capabilities
                .limit(LimitKey::MaxDynamicUniformBuffersPerPipelineLayout),
            max_dynamic_storage_buffers: capabilities
                .limit(LimitKey::MaxDynamicStorageBuffersPerPipelineLayout),
        }
    }
}
...
impl RasterLimits {
    ...
    pub(crate) fn from_capabilities(capabilities: &EnabledCapabilities) -> Self {
        Self {
            max_color_attachments: capabilities
                .limit(LimitKey::MaxColorAttachments)
                .unwrap_or(0),
            max_inter_stage_variables: capabilities
                .limit(LimitKey::MaxInterStageShaderVariables)
                .unwrap_or(0),
            max_bind_groups_plus_vertex_buffers: capabilities
                .limit(LimitKey::MaxBindGroupsPlusVertexBuffers),
            vertex: VertexInputLimits {
                max_buffers: capabilities.limit(LimitKey::MaxVertexBuffers).unwrap_or(0),
                max_attributes: capabilities.limit(LimitKey::MaxVertexAttributes).unwrap_or(0),
                max_stride: capabilities
                    .limit(LimitKey::MaxVertexBufferArrayStride)
                    .unwrap_or(0),
            },
        }
    }
...
impl RasterPipeline {
    ...
    /// The backing this handle retains, for the device's retirement registry.
    pub(crate) fn backing(&self) -> Arc<dyn RasterPipelineBackend> {
        Arc::clone(&self.inner)
    }
...
impl ComputePipeline {
    ...
    /// The backing this handle retains, for the device's retirement registry.
    pub(crate) fn backing(&self) -> Arc<dyn ComputePipelineBackend> {
        Arc::clone(&self.inner)
    }
...
impl ComputeLimits {
    ...
    pub(crate) fn from_capabilities(capabilities: &EnabledCapabilities) -> Self {
        Self {
            max_size_x: capabilities
                .limit(LimitKey::MaxComputeWorkgroupSizeX)
                .unwrap_or(0),
            max_size_y: capabilities
                .limit(LimitKey::MaxComputeWorkgroupSizeY)
                .unwrap_or(0),
            max_size_z: capabilities
                .limit(LimitKey::MaxComputeWorkgroupSizeZ)
                .unwrap_or(0),
            max_invocations: capabilities
                .limit(LimitKey::MaxComputeInvocationsPerWorkgroup)
                .unwrap_or(0),
            max_workgroup_storage: capabilities
                .limit(LimitKey::MaxComputeWorkgroupStorageSize)
                .unwrap_or(0),
        }
    }
}
