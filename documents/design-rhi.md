...
| 10. Primary-buffer mapping and persistent mapping leases | `MapMode`, exact `BufferSupportQuery`, range RAII, flush/invalidate, `MappablePrimaryBuffers`, `DeviceIdentity`, completion/loss | Every backend may publish ordinary staging shapes (`MAP_READ | COPY_DST`, `MAP_WRITE | COPY_SRC`) when its exact buffer facts and map lowering uphold them. `MappablePrimaryBuffers` is published only when map usage may additionally coexist with broader primary GPU usages; `PersistentMapping` remains unadvertised until its ownership contract is met. | An unsupported exact usage mask, range, coherence operation or persistent lease is rejected before native map; ordinary mapping is never rejected merely because the broader-primary feature is absent. No hidden staging or implicit CPU/GPU race is permitted. | Probe legal heap/memory type and coherency; enable/select it at allocation; wait for last GPU use before grant; lower atom-aligned Vulkan flush/invalidate or truthful DX12 coherence; retain the map lease and wake/cancel it on loss; pass staging, primary-combination, exact-end, non-coherent, overlap and loss conformance. |
