...
creates nothing of its own. Step 14's portable floor followed: `native::vulkan::limits`
holds the baseline an adapter must report, and `open::open` applies it after the adapter's
facts are read and **before** `vkCreateDevice`, so an adapter that cannot hold the RHI's
fixed recipes refuses with nothing created. The floor is the one the borrowed path
already applied through `wgt::Limits::default()`, restated in this backend's vocabulary
rather than invented, and a refusal becomes the same public
