...
| `api::{graphics, families}` | vocabulary complete; `Vulkan` implements `GraphicsApi`, `CopyApi`, `ComputeApi` and `StorageBufferApi` (`native::vulkan::family`, step 13) |
...
and the two stale-id refusals are values that leave the recording usable. The
storage-texture family, the indirect-dispatch family and the execution-layer
migration remain owed by step 13.
Clippy is clean
