...
| `api::{graphics, families}` | vocabulary complete; `Vulkan` implements `GraphicsApi`, `CopyApi`, `ComputeApi`, `StorageBufferApi` and `StorageTextureApi` (`native::vulkan::family`, step 13) |
...
storage texture and a stale generation are each refused by name as values. The
indirect-dispatch family and the execution-layer migration remain owed by step 13.
Clippy is clean
