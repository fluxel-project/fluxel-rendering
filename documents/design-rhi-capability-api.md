...
assert the eight feature-gated or preserved rows (`StorageBuffer`, `StorageImage`, `IndirectDraw`,
`MultiDrawIndirect`, `AnisotropicFiltering`, `Multiview`, `AsyncCompute`, `TransferQueue`) stay
both disabled and unexamined; against a real adapter the created device proves copy, proves
indirect dispatch exactly where the family reports compute, and proves timestamps exactly where
the family's valid-bit report is non-zero. Clippy is clean
